...
export const LiveSupportChatWidget: React.FC = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setActiveTicket(null); // Clear previous user's ticket immediately
      ...
      if (user) {
        ...
        try {
          ...
          if (isMounted && tickets && tickets.length > 0) {
            // Strictly match current user ID
            const userOwned = tickets.filter((t) => t.userId === user.id);
            if (userOwned.length > 0) {
              setActiveTicket(userOwned[0]);
            }
          }
        } catch {}
      } else {
        ...
        if (storedTicketId && storedEmail) {
          try {
            const ticket = await api.getGuestSupportTicket(storedTicketId, storedEmail);
            if (isMounted && ticket && ticket.id === storedTicketId) {
    ...
    const unsub1 = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          // Verify ownership before setting
    ...
    const unsub = onSnapshot(
      ...
      (snapshot) => {
        if (!snapshot.empty) {
          ...
          if (docs.length > 0) {
            setActiveTicket((prev) => {
              if (!prev || prev.userId !== currentUserId) return docs[0];
  ...
  // 3. Fast Revalidation Polling (every 3s) when chat is open
  useEffect(() => {
    if (!isOpen) return;
      ...
      try {
        if (user) {
          ...
          if (tickets && tickets.length > 0) {
            ...
            if (userOwned.length > 0) {
              setActiveTicket((prev) => {
                if (!prev || prev.userId !== user.id) return userOwned[0];
                const match = userOwned.find((t) => t.id === prev.id);
                return match || userOwned[0];
              });
            }
          }
        } else {
  ...
  useEffect(() => {
    const handleTicketUpdated = (e: Event) => {
      ...
      if (customEvent.detail) {
        ...
        if (user && updated.userId !== user.id) return;
        setActiveTicket((prev) => {
  ...
  const handleOpenLiveAgentMail = () => {
    window.location.href = 'mailto:help.netbybit@hotmail.com?subject=NETBYBIT%20Live%20Agent%20Inquiry';
  };
          ...
          {!isMinimized && (
            <div className="bg-amber-500/10 border-b border-amber-500/20 px-3 py-1.5 flex items-center justify-between text-[10px] text-amber-300 shrink-0">
              <div className="flex items-center space-x-1.5 font-mono truncate mr-1">
                ...
                <span className="font-bold text-amber-300 truncate">help.netbybit@hotmail.com</span>
              </div>
